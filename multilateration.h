...
/**@brief Perform 2D linear regression with zero intercept.
 *
 * @details Find the X and Y that minimize the sum (over i) of (x[i]*X+y[i]*Y-r[i])^2
 * for given arrays x[], y[], and r[]. Implemented for reference and testing purposes.
 *
 * @param[in] n    The length of the arrays x[i], y[i], and r[i].
 *
 * @param[in] x
 *
 * @param[in] y
 ...
 */
uint32_t linreg(uint32_t n, const float x[], const float y[], const float r[], float* X, float* Y)
{
...
/**@brief Perform 2D multilaterateration through linear regression.
 *
 * @details For location arrays x[] and y[] and corresponding distance array r[]
 ...
 * @param[out] Y
 *
 * @param[in] start    Starting point in the ring buffer arrays.
 *
 * @param[in] stop    End point in the ring buffer arrays.
 *
 * @retval 0 if successful, see multilateration_ringbuffer for failure codes
 */
uint32_t multilateration_ringbuffer(uint32_t n, const float x[], const float y[], const float r[], float* X, float* Y, uint32_t start, uint32_t stop)
{
    float sumx2 = 0.0;                     /* sum of x**2  */
    float sumxy = 0.0;                     /* sum of x * y */
    float sumy2 = 0.0;                     /* sum of y**2  */
 ...
 */
uint32_t multilateration_wholearray(uint32_t n, const float x[], const float y[], const float r[], float* X, float* Y)
{
    return multilateration_ringbuffer(n,x,y,r,X,Y,0,n-1);
}

/**@brief Multilateration state structure. This structure contains the current radio signal strength log and location data. */
...
/**@brief Initialize the multilateration state structure. It dynamically allocates the record buffer. */
...
/**@brief Uninitialize the multilateration state structure. It frees the record buffer. */
void multilateration_uninit(multilateration_state_t* state)
{
    free(state->x);
}

/**@brief Add a new record to the multilateration buffer. */
...
/**@brief Remove records older than a given limit value from the multilateration buffer. */
...
/**@brief Perform 2D multilaterateration through linear regression.
 *
 * @details It uses the data stored in the state
 *
