...
/* Measure and remove bias on the fly. Returns 1 if the magnetometer is not ready. */
...
/* Update the advert packet to contain motion data. */
static void mpu_update_advert()
{
...
/* Use the accel, gyro, and compas values from the sensor buffer to run sensor fusion. It performs simple bias removal. */
...
/* Use the accel and gyro values from the sensor buffer to run sensor fusion. It performs simple bias removal. */
...
/* Check the AK8963 readings. Call sensor fusion accordingly. To be set as callback from inside check_mag_ready_cb. If this is not fast, everything breaks. */
...
/* Check if the AK8963 said it is ready. Read it and call sensor fusion accordingly. To be set as callback from inside mpu_read_and_fuse_sensors. If this is not fast, everything breaks. */
...
/* Read all sensor data from the MPU9250. To be called from interrupt. */
...
/* Call whenvever the MPU9250's INT pin is triggered. */
